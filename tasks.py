...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_CC_PROVENANCE_xxx
namespace = Collection("nautobot_cc_provenance")
namespace.configure(
    {
        "nautobot_cc_provenance": {
            "nautobot_ver": "2.4.20",
            "project_name": "cc-provenance",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_cc_provenance.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_cc_provenance.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_cc_provenance.nautobot_ver,
        "PYTHON_VER": context.nautobot_cc_provenance.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_cc_provenance.project_name}",
        f'--project-directory "{context.nautobot_cc_provenance.compose_dir}"',
    ]

    for compose_file in context.nautobot_cc_provenance.compose_files:
        compose_file_path = os.path.join(context.nautobot_cc_provenance.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_cc_provenance.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_cc_provenance.python_ver}...")
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.nautobot_cc_provenance.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_cc_provenance.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_cc_provenance.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_cc_provenance.python_ver
    dockerfile_path = os.path.join(context.nautobot_cc_provenance.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_cc_provenance.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_cc_provenance"
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_cc_provenance'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_cc_provenance")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_cc_provenance.migrations"
    ...
    if migrations_dir.is_dir():
        if run_migrations_check:
            migrations_pylint_command = (
                ...
                " nautobot_cc_provenance.migrations"
            )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_cc_provenance/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_cc_provenance.local):
